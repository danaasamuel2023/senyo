        ...
        <div className="space-y-4">
          <div>
            ...
            <input
              ...
              className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-[#FFCC08] focus:border-transparent"
            />
          </div>
            ...
            onClick={async () => {
              ...
              try {
                ...
                if (response.ok) {
                  const result = await response.json();
                  if (result.success) {
                    showNotification('Paystack settings saved successfully', 'success');
                    setPaymentGatewaySettings(result.data);
                  } else {
