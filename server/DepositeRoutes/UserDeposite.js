...
// Add webhook endpoint for compatibility
router.get('/paystack-webhook', (req, res) => {
  res.status(405).json({
    success: false,
    error: 'Method not allowed. Use POST for webhook processing.'
...
// Add new wallet-based deposit endpoint for compatibility
router.post('/wallet-deposit', async (req, res) => {
  try {
    ...
    // Initiate Paystack payment with the total amount including fee
    const paystackAmount = Math.round(parseFloat(amount) * 100); // Convert to pesewas
